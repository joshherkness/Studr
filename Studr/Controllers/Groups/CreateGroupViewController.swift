...
class CreateGroupViewController : FormViewController {
    ...
    func createGroup(){
        ...
        groupMembersRef.childByAppendingPath(myId).setValue(MembershipStatus.Admin.rawValue)
        myMembershipsRef.childByAppendingPath(groupId).setValue(MembershipStatus.Admin.rawValue)
        
        // Add each member to the group
        for user in members {
            let membershipsRef = Database.MEMBERSHIP_REF.childByAppendingPath(user.uid)
            
            groupMembersRef.childByAppendingPath(user.uid).setValue(MembershipStatus.PendingMember.rawValue)
            membershipsRef.childByAppendingPath(groupId).setValue(MembershipStatus.PendingMember.rawValue)
        }
